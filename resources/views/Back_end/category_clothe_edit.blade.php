...
<body>
    <div class="container mt-5">
        <div class="card">
            ...
            តើអ្នកពិតជាចង់កែប្រែប្រភេទសម្លៀកបំពាក់មែនទេ?
            </div>
            <div class="card-body">
                <form action="{{ route('Back_end.category_clothe_update' , $category_clothes->id) }}" method="post">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
                        <label for="exampleInputEmail1" class="form-label">ប្រភេទសម្លៀកបំពាក់</label>
                        <input value="{{$category_clothes->text_clothes}}" type="text" name="text_clothes" class="form-contro input_searchl" id="exampleInputEmail1" aria-describedby="emailHelp">
                    </div>
                    <div class="mb-3">
                        <label for="exampleInputEmail1" class="form-label">សម្លៀកបំពាក់</label>
                        <!--<input type="text" name="clothe_id" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp">-->

                        <select class="form-control input_search" name="clothe_id" id="exampleInputEmail1" aria-describedby="emailHelp">
                        @foreach ($clothes as $st)
                            
                                <option name="clothe_id" value="{{$st->id}}">{{$st->text}}</option>
