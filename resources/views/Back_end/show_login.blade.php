...
<head>
    ...
    <title>Login - Men Fashion Store</title>
...
<body>
    <div class="box">
        ...
        <form action="{{ route('Back_end.login') }}" method="POST">
            @csrf
            
            <!-- ១. ប្រអប់ឈ្មោះ -->
            <div class="form-group">
                <label><i class="fas fa-user-shield"></i> ឈ្មោះអ្នកប្រើប្រាស់</label>
                <input class="four_corner" type="text" name="name" required placeholder="បញ្ចូលឈ្មោះរបស់" value="{{ old('name') }}" autofocus>
            </div>

            <!-- ២. ប្រអប់លេខទូរស័ព្ទ -->
            <div class="form-group">
                <label><i class="fab fa-telegram-plane"></i> លេខសម្គាល់តេឡេក្រាម</label>
                <input class="four_corner" type="text" name="phone_number" required placeholder="បញ្ចូលលេខសម្គាល់តេឡេក្រាម" value="{{ old('phone_number') }}">
            </div>

            <!-- ៣. ប្រអប់ពាក្យសម្ងាត់ -->
            <div class="form-group">
                <label><i class="fas fa-lock"></i> ពាក្យសម្ងាត់</label>
                <div style="position: relative;">
                    <input class="four_corner" type="password" name="password" id="password_login" required placeholder="បញ្ចូលពាក្យសម្ងាត់">
                    <i class="far fa-eye" id="togglePasswordLogin" style="position: absolute; right: 12px; top: 50%; transform: translateY(-50%); cursor: pointer; "></i>
                </div>
            </div>

            <button type="submit" class="btn">ចូលគ្រប់គ្រងប្រព័ន្ធ</button>
        </form>
        <a href="{{ route('Back_end.show_form_for_forgot') }}" id="back-back" class="btn btn-dark d-flex justify-content-center" style="margin-top:5px; " onclick="history.back()">ភ្លេចពាក្យសម្ងាត់</a>
    </div>
