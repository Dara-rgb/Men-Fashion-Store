...
<body>
    <div class="container-fluid bg_secondary w-25 position-fixed  sidebar-menu " >
        ...
        <ul class="list-unstyled text-center w-100 mt-1 ">
            ...
            <form class="form-inline mb-5 mt-4 sticky-top bg_search" action="{{ route('Back_end.search_contact') }}" method="GET">
            ...
            <li class="type_bg_data"><a class="type_text_data text-decoration-none d-block py-3 " href="{{route('Back_end.contact')}}">ទំនាក់ទំនង</a></li>
        ...
        <div class="container-fluid w-75 mr-0" >
            ...
            <div class="row mb-2 bg-data sticky-top w-100">
                <div class="col-12">
                    <h2 class="mt-2 mb-3 dark">បញ្ជីទំនាក់ទំនង</h2>
                    ...
                    <a href="{{ route('Back_end.contact_create') }}" class="text-decoration-none"><button class="btn btn-info text-btn" >បញ្ចូល</button> </a>
                    <a href="{{ route('Back_end.contact_alldestroy') }}" class="text-decoration-none"><button class="btn btn-warning text-btn" >លុបទាំងអស់</button> </a>
                </div>
            </div>
            <div class="row mt-1">
                @if (session('success'))
            ...
            <div class="row table-wrapper " style="margin-top: -7.65px;">
                <div class="col-12 ">
                    <table class="table table-bordered  table-min-width-contact" style="width:2000px;">
                        <thead class="table-sticky table-bg-color " >
                            <tr>
                                ...
                                <th scope="col" class="text-center table-color-thead-r" style="width:300px;">លេខសំគាល់តេឡេក្រាម</th>
                                
                                <th scope="col" colspan="2" class="table-color-thead-l text-center" style="width:300px;">ប៊ូតុង</th>
                            </tr>
                        </thead>
                        <tbody>
                            <p hidden>{{$id = 1;}}</p>
                            @foreach ($contact as $st)
                                ...
                                <tr class="table-text-color">
                                    ...
                                    <td class="table-color"> <p class="text-center">{{$st->name_contact}}</p>  </td>
                                    <td class="table-color"> <p class="text-center">{{$st->phone_contact}}</p>  </td>
                                    ...
                                    @php
                                        $total = $contactTotals[$st->id] ?? 0;
                                    @endphp
                                    <td class="table-color"> <a class="text-dark text-decoration-none" href="{{$st->link_payment}}{{ number_format($total, 2) }}"> <p class="text-center" style="display: -webkit-box;-webkit-line-clamp: 6;line-clamp: 6;-webkit-box-orient: vertical;text-overflow: ellipsis;overflow: hidden;">{{$st->link_payment}} {{ number_format($total, 2) }}</p> </a> </td>
                                    <td class="table-color"> <p class="text-center">{{$st->telegram_id}}</p>  </td>
