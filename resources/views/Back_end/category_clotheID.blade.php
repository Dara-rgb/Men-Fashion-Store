...
<body>
    <div class="container-fluid bg_secondary w-25 position-fixed  sidebar-menu ">
        ...
        <ul class="list-unstyled text-center w-100 mt-1 ">
            ...
            <form class="form-inline mb-5 mt-4 sticky-top bg_search" action="{{ route('Back_end.search_category_clothe') }}" method="GET">
            ...
            <li class="type_bg_data"><a class="type_text_data text-decoration-none d-block py-3 " href="{{route('Back_end.clothe')}}">សម្លៀកបំពាក់</a></li>
    ...
    <div class="container-fluid w-75 mr-0">
        ...
        <div class="row mb-2 sticky-top bg-data">
            <div class="col-12">
                <h2 class="mt-2 mb-3 dark">បញ្ជីប្រភេទសម្លៀកបំពាក់</h2>
                ...
                <a href="{{route('Back_end.category_clotheID_create',['id' => $clothes->id ])}}" class="text-decoration-none"><button class="btn btn-info text-btn" >បញ្ចូល</button> </a>
                <a href="{{route('Back_end.category_clotheID_alldestroy',['id' => $clothes->id ])}}" class="text-decoration-none"><button class="btn btn-warning text-btn" >លុបទាំងអស់</button> </a>
                <button onclick="history.back()" class="btn btn-darkk float-right" >ត្រឡប់ក្រោយ</button> 
            </div>
        </div>
        <div class="row mt-1">
            @if (session('success'))
        ...
        <table class="table table-bordered mt-1 ">
            <thead class="sticky-top table-bg-color " style="top: 99.30px;">
                <tr>
                    ...
                    <th class="table-color-thead-r text-center" scope="col">ប្រភេទសម្លៀកបំពាក់</th>
                    <th class="table-color-thead-r text-center" scope="col">សម្លៀកបំពាក់</th>
                    <th scope="col" colspan="2" class="table-color-thead-l text-center">ប៊ូតុង</th>
                </tr>
            </thead>
            <tbody>
                <p hidden>{{$id = 1;}}</p>
                @foreach ($category_clothes as $st)


                    <tr class="table-text-color">
                        <th class="text-center table-color" scope="row">{{$id++}}</th>
                        <td class="text-center table-color"><a class="text-dark text-decoration-none" href="{{route('Back_end.postID_category_clothe',['id' => $st->id])}}">{{$st->text_clothes}}</a></td>
                        <td class="text-center table-color">{{$st->clothe->text}}</td>
                        <td class="text-center table-color">
                            <a href="{{route('Back_end.category_clotheID_edit',['id' => $st->id ,'ID' => $st->clothe_id])}}"><button class="btn btn-sm btn-success  ">កែប្រែ</button></a>
                        </td>
                        <td class="text-center table-color">
                            <a href="{{route('Back_end.category_clotheID_destroy',['id' => $st->id ,'ID' => $st->clothe_id])}}"><button class="btn btn-sm btn-danger  ">លុប</button></a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
