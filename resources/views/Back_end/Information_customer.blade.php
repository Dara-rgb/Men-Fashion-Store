...
<body>
    <div class="container-fluid bg_secondary w-25 position-fixed  sidebar-menu ">
        ...
        <ul class="list-unstyled text-center w-100 mt-1 ">
            ...
            <form class="form-inline mb-5 mt-4 sticky-top bg_search" action="{{ route('Back_end.search_Information_customer') }}" method="GET">
            ...
            <li class="type_bg_data"><a class="type_text_data text-decoration-none d-block py-3 " href="{{route('Back_end.Information_customer')}}">ព័ត៍មានអតិថិជន</a></li>
        ...
        <div class="container-fluid mr-0 w-75" >
            ...
            <div class="row mb-2 sticky-top bg-data">
                <div class="col-12">
                    <h2 class="mt-2 mb-3 dark">បញ្ជីព័ត៌មានអតិថិជន</h2>
                    ...
                    <a href="{{route('Back_end.Information_customerr_alldestroy')}}" class="text-decoration-none"><button class="btn btn-warning text-btn" >លុបទាំងអស់</button> </a>
                </div>
            </div>
            <div class="row mt-1">
                @if (session('success'))
                    ...
                    <table class="table table-bordered table-min-width " style="width:3000px;">
                        <thead class="table-sticky table-bg-color">
                            <tr>
                                ...
                                <th scope="col" class=" table-color-thead-r text-center" >ចំនួនទំនិញ</th>
                                <th scope="col" class=" table-color-thead-r text-center" >ពេលចុចទិញ</th>
                                <th scope="col" colspan="3" class="table-color-thead-l text-center" style="width:150px;">ប៊ូតុង</th>
                            </tr>
                        </thead>
                        <tbody>
                            <p hidden>{{$id = 1;}}</p>
                            @foreach ($information_customers as $st)
                                ...
                                <tr class="table-text-color ">
                                    ...
                                    <td class=" table-color text-center">{{ number_format($st->picture->post->price * $st->number_items, 2) }}$</td>
                                    ...
                                    <td class=" table-color text-center" style="">{{$st->number_items}}</td>
                                    <td class=" table-color text-center" style="">{{$st->created_at}}</td>
                                    <td class="text-center table-color">
                                        <a href="{{route('Back_end.Information_customer_edit',['id' => $st->id ,'ID' => $st->picture_id])}}"><button class="btn btn-sm btn-success  ">កែប្រែ</button></a>
                                    </td>
                                    <td class="text-center table-color">
                                        <a href="{{route('Back_end.Information_customerr_destroy',['id' => $st->id ,'ID' => $st->picture_id])}}"><button class="btn btn-sm btn-danger  ">លុប</button></a>
                                    </td>
                                    <td class="text-center table-color">
                                        @if($st->is_sent)
                                            <a class="btn btn-sm btn-secondary" style="cursor: not-allowed;" disabled>
                                                <i class="fas fa-check-circle"></i> បានផ្ញើរួច
                                            </a>
                                        @else
                                            <a href="{{ route('Back_end.send_telegram', ['id' => $st->id]) }}" class="btn btn-sm btn-primary" onclick="singleClick(event, this)">
                                                <i class="fab fa-telegram-plane"></i> ផ្ញើចេញ
                                            </a>
                                        @endif
                                    </td>
                                </tr>
...
<script>
function singleClick(event, element) {
    // ១. បញ្ឈប់ការចុចស្ទួន (Disable the button)
    element.style.pointerEvents = "none"; // ធ្វើឱ្យលែងទទួលការចុច
    element.style.opacity = "0.5";        // ធ្វើឱ្យពណ៌ស្រអាប់
    
    // ២. ប្តូរអត្ថបទ និងដាក់រូបវិលៗ (Loading Spinner)
    element.innerHTML = '<i class="fas fa-spinner fa-spin"></i> កំពុងផ្ញើ...';
    
    // ទុកឱ្យលីង (a href) ដើរទៅកាន់ Controller ធម្មតា
    return true;
}
</script>