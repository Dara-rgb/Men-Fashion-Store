...
<body>
    <div class="container mt-5">
        <div class="card">
            ...
            តើអ្នកពិតជាចង់កែប្រែប្រភេទសាច់ក្រណាត់មែនទេ?
            </div>
            <div class="card-body">
                <form action="{{ route('Back_end.category_fabric_update' , $category_fabrics->id) }}" method="post">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
                        <label for="exampleInputEmail1" class="form-label">ប្រភេទសាច់ក្រណាត់</label>
                        <input value="{{$category_fabrics->text_fabrics}}" type="text" name="text_fabrics" class="form-control input_search" id="exampleInputEmail1" aria-describedby="emailHelp">
                    </div>
