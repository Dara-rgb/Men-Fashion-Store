...
<head>
    <meta charset="UTF-8">
    <title>Edit - Men Fashion Store</title>
...
<body>
    <div class="box">
        ...
        <h2>កែប្រែព័ត៌មានសម្ងាត់</h2>
        <form action="{{ route('Back_end.security_update', $users->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="form-group">
                <label>ឈ្មោះ</label>
                <input value="{{$users->name}}" class="four_corner" type="text" name="name" required placeholder="ឈ្មោះ">
            </div>
            <div class="form-group">
                <label>លេខសម្គាល់តេឡេក្រាម</label>
                <input value="{{$users->phone_number}}" class="four_corner" type="text" name="phone_number" required placeholder="លេខសម្គាល់តេឡេក្រាម">
            </div>
            <div class="form-group">
                <label>ពាក្យសម្ងាត់</label>
                <div style="position: relative;">
                    <input value="{{$users->password}}" class="four_corner" type="password" name="password" id="password" required placeholder="វាយពាក្យសម្ងាត់...">
