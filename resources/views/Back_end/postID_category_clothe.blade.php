...
<body>
    <div class="container-fluid bg_secondary w-25 position-fixed  sidebar-menu ">
        ...
        <ul class="list-unstyled text-center w-100 mt-1 ">
            ...
            <form class="form-inline mb-5 mt-4 sticky-top bg_search" action="{{ route('Back_end.search_post') }}" method="GET">
            ...
            <li class="type_bg_data"><a class="type_text_data text-decoration-none d-block py-3 " href="{{route('Back_end.category_clothe')}}">ប្រភេទសម្លៀកបំពាក់</a></li>
    ...
    <div class="container-fluid w-75 mr-0 ">
            ...
            <div class="row mb-2 sticky-top bg-data">
                <div class="col-12">
                    <h2 class="mt-2 mb-3 dark">បញ្ជីការបង្ហោះ</h2>
                    ...
                    <a href="{{route('Back_end.postID_category_clothe_create',['id' => $category_clothes->id ])}}" class="text-decoration-none"><button class="btn btn-info text-btn" >បញ្ចូល</button> </a>
                    <a href="{{route('Back_end.postID_category_clothe_alldestroy',['id' => $category_clothes->id ])}}" class="text-decoration-none"><button class="btn btn-warning text-btn" >លុបទាំងអស់</button> </a>
                    <button onclick="history.back()" class="btn btn-darkk float-right turn-back" >ត្រឡប់ក្រោយ</button> 
                </div>
            </div>
            <div class="row mt-1">
                @if (session('success'))
                ...
                <div class="col-12">
                    <table class="table table-bordered table-min-width " >
                        <thead class="table-sticky table-bg-color">
                            <tr>
                                <th scope="col" class=" table-color-thead-r text-center" style="width:85px;">លេខរៀង</th>
                                <th scope="col" class=" table-color-thead-r text-center">ខ្លឹមសារ</th>
                                <th scope="col" class=" table-color-thead-r text-center" style="width:75px;">រូបភាព</th>
                                <th scope="col" class=" table-color-thead-r text-center">តម្លៃ</th>
                                <th scope="col" class=" table-color-thead-r text-center">ម៉ាក</th>
                                <th scope="col" class=" table-color-thead-r text-center" style="width:146.5px;">ប្រភេទសម្លៀកបំពាក់</th>
                                <th scope="col" class=" table-color-thead-r text-center" style="width:146.5px;">ប្រភេទសាច់ក្រណាត់</th>
                                <th scope="col" class=" table-color-thead-r text-center" style="width:146.5px;">ចំនួនអ្នកមើល</th>
                                <th scope="col" colspan="2" class="table-color-thead-l text-center" style="width:3000px;">ប៊ូតុង</th>
                            </tr>
                        </thead>
                        <tbody>
                            <p hidden>{{$id = 1;}}</p>
                            @foreach ($post as $st)
                                ...
                                <tr class="table-text-color">
                                    <th scope="row" class=" text-center table-color">{{$id++}}</th>
                                    <td class="text-center table-color"><p  style="display: -webkit-box;-webkit-line-clamp: 6;line-clamp: 6;-webkit-box-orient: vertical;text-overflow: ellipsis;overflow: hidden;">{{$st->caption}}</p></td>
                                    <td class="table-color"> <a href="{{route('Back_end.pictureID_post',['id' => $st->id])}}"><img src="{{ asset('storage/' . $st->image) }}" alt="" height="150" width="150"></a>  </td>
                                    <td class="table-color"><p class="text-center" style="display: -webkit-box;-webkit-line-clamp: 6;line-clamp: 6;-webkit-box-orient: vertical;text-overflow: ellipsis;overflow: hidden; margin-left:-10px;">{{$st->price}}$</p></td>
                                    <td class=" text-center table-color">{{$st->brand}}</td>
                                    <td class=" text-center table-color">{{$st->category_clothe->text_clothes}}</td>
                                    <td class=" text-center table-color">{{$st->category_fabric->text_fabrics}}</td>
                                    <td class=" text-center table-color">{{$st->view}}</td>
                                    <td class="text-center table-color">
                                        <a href="{{route('Back_end.postID_category_clothe_edit',['id' => $st->id ,'Id' => $st->category_clothe_id ,'ID' => $st->category_fabric_id])}}"><button class="btn btn-sm btn-success  ">កែប្រែ</button></a>
                                    </td>
                                    <td class="text-center table-color">
                                        <a href="{{route('Back_end.postID_category_clothe_destroy',['id' => $st->id ,'Id' => $st->category_clothe_id ,'ID' => $st->category_fabric_id])}}"><button class="btn btn-sm btn-danger  ">លុប</button></a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
